...
internal sealed partial class ScanDialog : CustomForm
{
    ...
    {
        IEnumerable<(Platform platform, string id)> storedChoices = ProgramData.ReadProgramChoices();
        if (storedChoices is null)
            return;
        List<(Platform platform, string id)> choices = storedChoices.ToList();
        if (choices.Count < 1)
            return;
        foreach (TreeNode node in selectionTreeView.Nodes)
